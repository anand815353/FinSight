...
from tests.test_document_chunk_repo_fakes import _FakeMongoManager as _ChunkMongo
from tests.test_processing_job_repo_fakes import _FakeMongoManager as _ProcessingJobMongo
...
def test_processing_job_repo_index_definitions(monkeypatch):
    monkeypatch.setenv("APP__SECRET_KEY", "supersecret")
    mongo = _ProcessingJobMongo()
    repo = ProcessingJobRepository(mongo, Settings())
    asyncio.run(repo.ensure_indexes())
    keys = [call[0] for call in mongo._client.document_processing_jobs.index_calls]
    assert [("_id", 1)] in keys
    assert ("document_id", 1) in keys[1]


def test_document_chunk_repo_index_definitions(monkeypatch):
    monkeypatch.setenv("APP__SECRET_KEY", "supersecret")
    mongo = _ChunkMongo()
    repo = DocumentChunkRepository(mongo, Settings())
    asyncio.run(repo.ensure_indexes())
    keys = [call[0] for call in mongo._client.document_chunks.index_calls]
    assert [("_id", 1)] in keys
    assert ("document_id", 1) in keys[1]
